...
{
    public EqualsCondition(Expression left, Expression right, int rowNumber) : base(left, right, rowNumber) { }
    ...
    protected override void CheckTypes(Type leftType, Type rightType)
    {
        ...
        if (leftType != typeof(string) && leftType != typeof(int) && leftType != typeof(double) && leftType != typeof(bool))
            throw new Exceptions.InvalidDatatypeException("Equals to condition is defined only for String, Int, Double, Boolean datatypes", RowNumber);
    }
